...
public class Main {
    ...
    private static void initInfected() {
        if (Factors.INITINFECTED_MODEL == 1) {
        ...
        } else if (Factors.INITINFECTED_MODEL == 2) {
            ...
            for (int i = 0; i < 20; i++) {
                ...
                } while (person.isInfected() || (person.getX() > 230 ));
                person.beInfected();
            }
            //central section
            for (int i = 0; i < 20; i++) {
            ...
            for (int i = 0; i < 20; i++) {
                ...
                do {
                    person = people.get(new Random().nextInt(people.size() - 1));
                } while (person.isInfected() || (person.getX() < 460));
                person.beInfected();
            }
        } else if (Factors.INITINFECTED_MODEL == 3) {
          //set infection source in the south
         List<Person> people = Population.getInstance().getPersonList();
        for (int i = 0; i < 20; i++) {
            Person person;
            do {
